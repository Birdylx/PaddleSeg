...
class StrideFormer(nn.Layer):
    ...
    def __init__(self,
        ...
        for i in range(len(cfgs)):
            smb = StackedMV3Block(cfgs=cfgs[i],
        ...
        if self.inj_type == "AAM":
            ...
            for i in range(len(dims)):
                fuse = Fusion_block(out_feat_chs[0] if i == 0 else dims[i - 1],
                                    out_feat_chs[i + 1],
                                    embed_dim=dims[i])
...
class ConvBNLayer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.c = nn.Conv2D(in_channels=in_c,
                           out_channels=out_c,
    ...
    def __init__(self, cfgs, stem, inp_channel, scale=1.0):
        ...
        if self.stem:
            self.conv = ConvBNLayer(in_c=3,
                                    out_c=_make_divisible(inp_channel *
                                                          self.scale),
                                    filter_size=3,
                                    stride=2,
                                    padding=1,
        ...
        for i, (k, exp, c, se, act, s) in enumerate(cfgs):
            self.blocks.append(
                ResidualUnit(in_c=_make_divisible(inp_channel * self.scale),
...
class SqueezeAxialPositionalEmbedding(nn.Layer):
    ...
    def __init__(self, dim, shape):
        ...
        self.pos_embed = paddle.create_parameter(shape=[1, dim, shape],
                                                 attr=weight_attr,
                                                 dtype='float32')
...
class Sea_Attention(nn.Layer):
    ...
    def __init__(self,
        ...
        self.proj = paddle.nn.Sequential(
            activation(), Conv2DBN(self.dh, dim, bn_weight_init=0))
        self.proj_encode_row = paddle.nn.Sequential(
            activation(), Conv2DBN(self.dh, self.dh, bn_weight_init=0))
        ...
        self.dwconv = Conv2DBN(2 * self.dh,
                               2 * self.dh,
                               ks=3,
                               stride=1,
                               pad=1,
    ...
    def forward(self, x):
        ...
        qrow = self.pos_emb_rowq(q.mean(-1)).reshape(
            [B, self.num_heads, -1, H]).transpose([0, 1, 3,
                                                   2])  # [B, nhead, H, dim]
        ...
        vcolumn = paddle.mean(v, -2).reshape([B, self.num_heads, -1,
                                              W]).transpose([0, 1, 3, 2])
...
class InjectionMultiSumallmultiallsum(nn.Layer):
    ...
    def __init__(self, in_channels=(64, 128, 256, 384), out_channels=256):
        ...
        for i in range(len(in_channels)):
            self.embedding_list.append(
                ConvBNAct(in_channels[i], out_channels, kernel_size=1))
    ...
    def forward(self, inputs):  # x_x8, x_x16, x_x32, x_x64
        ...
        low_feat2 = F.interpolate(inputs[1],
        ...
        high_feat_act = F.interpolate(self.act_list[2](
            self.act_embedding_list[2](inputs[2])),
                                      size=low_feat2.shape[2:],
                                      mode="bilinear")
        high_feat = F.interpolate(self.embedding_list[2](inputs[2]),
                                  size=low_feat2.shape[2:],
                                  mode="bilinear")
...
class InjectionMultiSumallmultiallsumSimpx8(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        high_feat_act = F.interpolate(self.act_list[1](
            self.act_embedding_list[1](inputs[2])),
                                      size=low_feat2.shape[2:],
                                      mode="bilinear")
        high_feat = F.interpolate(self.embedding_list[1](inputs[2]),
                                  size=low_feat2.shape[2:],
                                  mode="bilinear")
...
def MobileSeg_Base(**kwargs):
    ...
    model = StrideFormer(cfgs=[cfg1, cfg2, cfg3, cfg4],
                         channels=channels,
                         embed_dims=emb_dims,
                         depths=depths,
                         num_heads=num_heads,
