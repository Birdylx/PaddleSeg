...
def get_reference_points(spatial_shapes):
    ...
    for _, (H_, W_) in enumerate(spatial_shapes):
        ref_y, ref_x = paddle.meshgrid(
            paddle.linspace(0.5, H_ - 0.5, H_, dtype='float32'),
...
def deform_inputs(x):
    _, _, h, w = x.shape
    spatial_shapes = paddle.to_tensor([(h // 8, w // 8), (h // 16, w // 16),
                                       (h // 32, w // 32)],
                                      dtype='int64')
...
class DWConv(nn.Layer):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        n = N // 21
        x1 = x[:, 0:16 * n, :].transpose([0, 2,
                                          1]).reshape([B, C, H * 2, W * 2])
        x2 = x[:, 16 * n:20 * n, :].transpose([0, 2, 1]).reshape([B, C, H, W])
        x3 = x[:, 20 * n:, :].transpose([0, 2,
                                         1]).reshape([B, C, H // 2, W // 2])
