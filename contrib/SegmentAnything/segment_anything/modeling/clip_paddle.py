...
class ResidualAttentionBlock(paddle.nn.Layer):
    ...
    def __init__(self,
        ...
        self.mlp = paddle.nn.Sequential(
            *[('c_fc',
               paddle.nn.Linear(in_features=d_model, out_features=d_model *
                                4)), ('gelu', QuickGELU()),
              ('c_proj',
               paddle.nn.Linear(in_features=d_model *
                                4, out_features=d_model))])
...
class VisionTransformer(paddle.nn.Layer):
    ...
    def __init__(self, input_resolution: int, patch_size: int, width: int,
        ...
        self.conv1 = paddle.nn.Conv2D(in_channels=3,
                                      ...
                                      bias_attr=False)
        scale = width**-0.5
        self.class_embedding = self.create_parameter(shape=(width, ),
        ...
        self.proj = self.create_parameter(shape=(width, output_dim),
                                          default_initializer=zeros_)
    ...
    def forward(self, x: paddle.Tensor):
        ...
        x = paddle.concat([
            ...
            paddle.zeros(shape=[x.shape[0], 1, x.shape[-1]], dtype=x.dtype), x
        ],
                          axis=1)
...
class CLIP(paddle.nn.Layer):
    ...
    def __init__(self, embed_dim: int, image_resolution: int,
                 ...
                 vocab_size: int, transformer_width: int,
        ...
        self.text_projection = self.create_parameter(shape=(transformer_width,
                                                            embed_dim),
                                                     default_initializer=zeros_)
    ...
    def encode_text(self, text):
        ...
        x = x[paddle.arange(start=x.shape[0]),
              text.argmax(axis=-1)] @ self.text_projection
        return x[None, :]
