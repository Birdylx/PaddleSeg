...
class Attention(nn.Layer):
    ...
    def __init__(
        ...
        self.qkv = nn.Linear(dim,
                             dim * 3,
                             bias_attr=qkv_bias if not qkv_bias else None)
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        q, k, v = qkv.reshape(shape=[3, B * self.num_heads, H *
                                     W, -1]).unbind(axis=0)
        ...
        x = (attn @ v).reshape([B, self.num_heads, H, W,
                                -1]).transpose([0, 2, 3, 1,
                                                4]).reshape([B, H, W, -1])
        x = self.proj(x)
...
def window_unpartition(windows: paddle.Tensor, window_size: int,
